...
public class ChongQingLottCheck extends Activity implements OnClickListener {
	private Intent mIntent;
	private LottBetWheel doubleWheel;
	private ChongQingLott inBet, clickBet, editBet;
	private ArrayList<ChongQingLott> betList;
	...
	private TextView billText, doubleBetText, followText, lotteryTypeName;
	private ImageView addBetHandly, addBetRandomly;
	private View footer, pay, corpView, doubleView, followView;
	private Handler mHandler;
	private View buyCorpView, followBox;
	private TextView phoneRules;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		phoneRules.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent1.setClass(ChongQingLottCheck.this, WebviewActivity.class);
...
//		followBox.setVisibility(View.INVISIBLE);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 0 && data != null) {
			inBet = (ChongQingLott) data.getSerializableExtra("sbet");
			if (null != inBet) {
				betList.add(inBet);
				betAdapter.notifyDataSetChanged();
			}
		} else if (requestCode == 1 && data != null) {
			editBet = (ChongQingLott) data.getSerializableExtra("sbet");
	...
	public void onClick(View v) {
		...
		case R.id.handicView:
			...
			Intent addBet = new Intent(this, ChongQingLottory.class);
			startActivityForResult(addBet, 0);
			break;
		case R.id.addBetRandomly:
		case R.id.randomView:
			ChongQingLott randomBet = new ChongQingLott();
			randomBet.setPlayType(2);
			ArrayList<Integer> redBalls = new ArrayList<Integer>();
			for (int i : RandomBallsUtils.getRandomBalls(1, 10)) {
				redBalls.add(i);
			}
		...
		case R.id.pay:
			...
			if (sutil.getLoginStatus()) {
				...
				for (int i = 0; i < betList.size(); i++) {
					Log.e("pkx",
							"重庆时时彩 betinfo："
									+ betList.get(i).getBetInfo(bet_double));
				}
				ChormBetAuth cb = getBetAuth();
				String chormMingwen = Net.gson.toJson(cb);
				String chromMiwen = MDUtils.MDEncode(sutil.getuserKEY(),
						chormMingwen);
				PublicAllAuth all = new PublicAllAuth("betCQSSC", chromMiwen);
				...
				Intent toLogin = new Intent(ChongQingLottCheck.this,
						LoginActivity.class);
				toLogin.putExtra("fastlogin", true);
				startActivityForResult(toLogin, Constant.FAST_LOGIN);
			}
	...
	private ChormBetAuth getBetAuth() {
		int count = 0;
		String infos = "";
		for (ChongQingLott lo : betList) {
			count += lo.getPrice() / 2;
			infos += lo.getBetInfo(bet_double) + "@@@";
		}
		infos = infos.substring(0, infos.length() - 3);
		ChormBetAuth cb = new ChormBetAuth(sutil.getuid(),
				String.valueOf((checkBill()/(bet_follow + 1))), String.valueOf(count),
				String.valueOf(bet_double), infos, "0", bet_follow + 1);
		return cb;
	}
	...
	private void initViews() {
		...
		betList = new ArrayList<ChongQingLott>();
		mIntent = getIntent();
		inBet = (ChongQingLott) mIntent.getSerializableExtra("sbet");
		if (null != inBet) {
			betList.add(inBet);
			checkBill();
		}
		...
		betListview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent addHand = new Intent(ChongQingLottCheck.this,
			...
			if (msg.what == Constant.POST_SUCCESS_STATUS_ONE) {
				...
				try {
					if ("10001".equals(jo.get("error").toString())) {
						...
						Toast.makeText(ChongQingLottCheck.this, "投注成功",
								Toast.LENGTH_SHORT).show();
						Intent toHome = new Intent(ChongQingLottCheck.this,
								HomeActivity.class);
						...
						Constant.alertWarning(ChongQingLottCheck.this,
								jo.get("message").toString());
						Toast.makeText(ChongQingLottCheck.this,
...
//								Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(ChongQingLottCheck.this, "请求超时",
						Toast.LENGTH_SHORT).show();
			} else if (msg.what == Constant.POST_SUCCESS_STATUS_ZERO) {
				Toast.makeText(ChongQingLottCheck.this, msg.obj.toString(),
						Toast.LENGTH_SHORT).show();
