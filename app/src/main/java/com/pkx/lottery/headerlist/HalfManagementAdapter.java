...
public class HalfManagementAdapter extends BaseExpandableListAdapter implements
		QQHeaderAdapter {
	...
	private ArrayList<TextView> clickText;
	private int fullChildposition, fullGroupPositin;

	public HalfManagementAdapter(Context context, ManagementListView listView,
	...
	private void alertFootballBet(Context ctx, int groupPos, int childPos,
		...
		alert.show();
		alert.getWindow().setContentView(R.layout.football_half_dialog);
		...
		d_we_text = (TextView) alert.findViewById(R.id.d_we_text);
		...
		ArrayList<TextView> halftexts = new ArrayList<TextView>();
		halftexts.add(d_ll_text);
		halftexts.add(d_le_text);
		halftexts.add(d_lw_text);
		halftexts.add(d_el_text);
		halftexts.add(d_ee_text);
		halftexts.add(d_ew_text);
		halftexts.add(d_wl_text);
		halftexts.add(d_we_text);
		halftexts.add(d_ww_text);
		for (int i = 0; i < halftexts.size(); i++) {
			halftexts.get(i).setText(
					Constant.MIX_DAYMATCHESS.get(groupPos).getMatchs()
							.get(childPos).getHalfCourt_odds().get(i));
		}
