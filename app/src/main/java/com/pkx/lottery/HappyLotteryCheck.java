...
public class HappyLotteryCheck extends Activity implements OnClickListener {
	private Intent mIntent;
	private LottBetWheel doubleWheel;
	private HappyTenLott inBet, clickBet, editBet;
	private ArrayList<HappyTenLott> betList;
	...
	private TextView billText, doubleBetText, phoneRules, followText,
			lotteryTypeName;
	private ImageView addBetHandly, addBetRandomly;
	private View footer, pay, corpView, doubleView, followView;
	private Handler mHandler;
	private View buyCorpView;
	private View followBox;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		phoneRules.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent1.setClass(HappyLotteryCheck.this, WebviewActivity.class);
	...
	protected void onResume() {
		...
		buyCorpView = findViewById(R.id.buyCorpView);
		buyCorpView.setVisibility(View.INVISIBLE);
		lotteryTypeName.setText("快乐十分");
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 0 && data != null) {
			inBet = (HappyTenLott) data.getSerializableExtra("sbet");
			if (null != inBet) {
				betList.add(inBet);
				betAdapter.notifyDataSetChanged();
			}
		} else if (requestCode == 1 && data != null) {
			editBet = (HappyTenLott) data.getSerializableExtra("sbet");
	...
	private ChormBetAuth getBetAuth() {
		int count = 0;
		String infos = "";
		for (HappyTenLott lo : betList) {
			count += lo.getPrice() / 2;
			infos += lo.getBetInfo(bet_double) + "@@@";
		}
		infos = infos.substring(0, infos.length() - 3);
		ChormBetAuth cb = new ChormBetAuth(sutil.getuid(),
				String.valueOf((checkBill() / (bet_follow + 1))),
				// String.valueOf(1),
				String.valueOf(count), String.valueOf(bet_double), infos, "0",
				bet_follow + 1);
		return cb;
	}
	...
	public void onClick(View v) {
		...
		case R.id.handicView:
			...
			Intent addBet = new Intent(this, HappyTenLottery.class);
			startActivityForResult(addBet, 0);
			break;
		case R.id.addBetRandomly:
		case R.id.randomView:
			Log.e("pkx", "happy random!!!!");
			HappyTenLott randomBet = new HappyTenLott();
			randomBet.setPlayType(1);
			ArrayList<Integer> redBalls = new ArrayList<Integer>();
			for (int i : RandomBallsUtils.getRandomBalls(2, 20)) {
				Log.e("pkx", "happy random!!!!for:" + i);
				redBalls.add(i + 1);
			}
		...
		case R.id.corpView:
			// if (bet_follow > 0) {
			// Toast.makeText(HappyLotteryCheck.this, "合买不能设置追号！",
			...
			// } else {
			// Intent toLogin = new Intent(HappyLotteryCheck.this,
			// LoginActivity.class);
		...
		case R.id.pay:
			...
			if (sutil.getLoginStatus()) {
				...
				ChormBetAuth cb = getBetAuth();
				Log.e("pkx", "追号" + (bet_follow + 1));
				for (HappyTenLott lo : betList) {
					Log.e("pkx", "betinfo:" + lo.getBetInfo(bet_double));
				}
				String chormMingwen = Net.gson.toJson(cb);
				String chromMiwen = MDUtils.MDEncode(sutil.getuserKEY(),
						chormMingwen);
				PublicAllAuth all = new PublicAllAuth("betKLSF", chromMiwen);
			...
			// final int mili=new Random().nextInt(2000);
			...
			// mHandler, Constant.NET_ACTION_SENCENBET);
			//
	...
	private int checkBill() {
		...
		return price * bet_double * (bet_follow + 1);

	}

	private void initViews() {
		followBox = findViewById(R.id.followBox);
		...
		lotteryTypeName = (TextView) findViewById(R.id.lotteryTypeName);
		...
		betList = new ArrayList<HappyTenLott>();
		mIntent = getIntent();
		inBet = (HappyTenLott) mIntent.getSerializableExtra("sbet");
		if (null != inBet) {
			betList.add(inBet);
			checkBill();
		}
		...
		betListview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// if(betList.get(position).getPlayType()>4&&betList.get(position).getPlayType()!=6){
				// return;
				// }
				...
				Intent addHand = new Intent(HappyLotteryCheck.this,
			...
			if (msg.what == Constant.POST_SUCCESS_STATUS_ONE) {
				...
				try {
					if ("10001".equals(jo.get("error").toString())) {
						...
						Toast.makeText(HappyLotteryCheck.this, "投注成功",
								Toast.LENGTH_SHORT).show();
						Intent toHome = new Intent(HappyLotteryCheck.this,
								HomeActivity.class);
						...
						Constant.alertWarning(HappyLotteryCheck.this,
								jo.get("message").toString());
						message = jo.get("message").toString();
						Toast.makeText(HappyLotteryCheck.this, message,
								Toast.LENGTH_LONG).show();
					}
				} catch (JSONException e1) {
					Toast.makeText(HappyLotteryCheck.this, "投注异常!",
							Toast.LENGTH_SHORT).show();
					e1.printStackTrace();
				}
				...
				Toast.makeText(HappyLotteryCheck.this, "请求超时",
						Toast.LENGTH_SHORT).show();
			} else if (msg.what == Constant.POST_SUCCESS_STATUS_ZERO) {
				Toast.makeText(HappyLotteryCheck.this, msg.obj.toString(),
						Toast.LENGTH_SHORT).show();
