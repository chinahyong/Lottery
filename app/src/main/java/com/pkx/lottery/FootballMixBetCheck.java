...
//likjas
public class FootballMixBetCheck extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		switch (checkType) {
		case 1:
			typeName.setText("胜平负");
			break;
		case 2:
			typeName.setText("让球胜平负");
			break;
		case 3:
			typeName.setText("全场比分");
			break;
		case 4:
			typeName.setText("总进球数");
			break;
		case 5:
			typeName.setText("半全场");
			break;
		case 6:
			typeName.setText("混合投注");
			break;
		}
		...
		for (int i = 0; i < Constant.MATCH_STATUS.size(); i++) {
			for (int j = 0; j < Constant.MATCH_STATUS.get(i).size(); j++) {
				Constant.MATCH_STATUS.get(i).get(j).set(63, 0);
			}
		...
		chuan6flag = true;
		chuan7flag = true;
		FOTTBAL_BETITEMS = RandomBallsUtils.changStatusToFootBet();
	...
	private int checkBill() {
		int betsNum = 0;
		if (chuans.get(0)) {
			betsNum += RandomBallsUtils.getBetNumberFromBets(FOTTBAL_BETITEMS,
					2, getDanNumber());
		}
		if (chuans.get(1)) {
			betsNum += RandomBallsUtils.getBetNumberFromBets(FOTTBAL_BETITEMS,
	...
	private void getAdapterContent() {
		...
		gPositions.clear();
		cPositions.clear();
		for (int groupPositin = 0; groupPositin < Constant.MATCH_STATUS.size(); groupPositin++) {
			for (int childPositon = 0; childPositon < Constant.MATCH_STATUS
					.get(groupPositin).size(); childPositon++) {
				if (Constant.MATCH_STATUS.get(groupPositin).get(childPositon)
						.get(64) == 1) {
					stringInfos.add(Constant.MATCH_INFOS.get(groupPositin).get(
							childPositon));
				}
				String matchInfo = "";
				boolean isSelect = false;
				for (int i = 0; i < 55; i++) {
					if (Constant.MATCH_STATUS.get(groupPositin)
							.get(childPositon).get(i) == 1) {
						matchInfo += Constant.FOOTBALL_BET_STRS[i] + ";";
						isSelect = true;
					}
				}
				if (isSelect) {
	...
	private void initViews() {
		...
		betAdapter = new BaseAdapter() {
			...
			public View getView(final int position, View convertView,
				...
				if (Constant.MATCH_STATUS.get(gPositions.get(position))
						.get(cPositions.get(position)).get(63) == 1) {
				...
				danImg.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (Constant.MATCH_STATUS.get(gPositions.get(position))
								.get(cPositions.get(position)).get(63) == 1) {
							Constant.MATCH_STATUS.get(gPositions.get(position))
									.get(cPositions.get(position)).set(63, 0);
							danImg.setImageResource(R.drawable.dan_normal);
							danIndex.remove(Constant.MIX_DAYMATCHESS
							...
							if (getDanNumber() == betInfos.size() - 1
									|| getDanNumber() == 7) {
								Toast.makeText(FootballMixBetCheck.this,
										"设胆已达上限", Toast.LENGTH_SHORT).show();
								return;
							}
							Constant.MATCH_STATUS.get(gPositions.get(position))
									.get(cPositions.get(position)).set(63, 1);
							danImg.setImageResource(R.drawable.dan_selected);
				...
				Log.e("pkx",
						"位置："
								...
								+ Constant.MATCH_STATUS
										...
										.get(cPositions.get(position)).get(63)
								+ "Gsize:"
								+ gPositions.size()
								+ "Csize"
								+ cPositions.size()
								...
								+ Constant.MATCH_STATUS
										.get(gPositions.get(position))
										.get(cPositions.get(position)).get(62));
				return convertView;
			}
		...
		betListview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent addHand = new Intent(FootballMixBetCheck.this,
						SevenLottery.class);
	...
	private int getDanNumber() {
		...
		for (int i = 0; i < Constant.MATCH_STATUS.size(); i++) {
			for (int j = 0; j < Constant.MATCH_STATUS.get(i).size(); j++) {
				if (Constant.MATCH_STATUS.get(i).get(j).get(63) == 1) {
					num++;
				}
	...
	private void alertMoneyLess() {
		...
		okButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent1.setClass(FootballMixBetCheck.this,
			...
			if (msg.what == Constant.POST_SUCCESS_STATUS_ONE) {
				...
				try {
					if ("10001".equals(jo.get("error").toString())) {
						...
						Toast.makeText(FootballMixBetCheck.this, "投注成功",
								Toast.LENGTH_SHORT).show();
						Intent toHome = new Intent(FootballMixBetCheck.this,
								HomeActivity.class);
						...
						Constant.alertWarning(FootballMixBetCheck.this,
				...
				Toast.makeText(FootballMixBetCheck.this, "请求超时",
						Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(FootballMixBetCheck.this, "投注失败",
						Toast.LENGTH_SHORT).show();
