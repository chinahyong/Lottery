...
import com.pkx.lottery.headerlist.PointsMatch;
import com.pkx.lottery.headerlist.PointsMatchInDay;
...
public class FootBallPoints extends Activity implements OnClickListener {
	private ManagementListView footballList;
	private PointsManagementAdapter myadapter;
	...
	private ArrayList<PointsMatchInDay> matchs;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.football_points);
		intiViews();
		mIntent = getIntent();
		matchs = (ArrayList<PointsMatchInDay>) mIntent
		...
		for (int i = 0; i < matchs.size(); i++) {
			...
			for (PointsMatch pma : matchs.get(i).getList()) {
				Match m = new Match();
				m.setSn_week(pma.getSn_week());
				m.setSn(pma.getSn());
				m.setMatchColor(pma.getMatchColor());
				m.setStatus(pma.getStatus());
				m.setScore_odds(pma.getScore_odds());
				m.setSn_date(pma.getSn_date());
				m.setMatch_id(pma.getMatch_id());
				m.setLetBall(pma.getLetBall());
				m.setRowNumber(pma.getRowNumber());
				m.setEndTime(pma.getEndTime());
				m.setPollCode(pma.getPollCode());
				m.setLocal_id(pma.getLocal_id());
				m.setUpdate_time(pma.getUpdate_time());
				m.setMainTeam(pma.getMainTeam());
				m.setMid(pma.getMid());
				m.setMatchName(pma.getMatchName());
				m.setPlayCode(pma.getPlayCode());
				m.setGuestTeam(pma.getGuestTeam());
				mas.add(m);
			}
			dms.setMatchs(mas);
			dayMatchses.add(dms);
		...
		myadapter = new PointsManagementAdapter(this, footballList,
				dayMatchses, handler);
