...
class ViewPropertyAnimatorPreHC extends ViewPropertyAnimator {
    /**
     * Proxy com.nineoldandroids.animation class which will allow us access to post-Honeycomb properties that were not
     * otherwise available.
     */
     ...
     * properties currently being animated, as well as the cleanup after an com.nineoldandroids.animation is
     * complete.
     */
     ...
     * property is tossed onto the pending list until the com.nineoldandroids.animation actually starts (which is
     ...
     * a property com.nineoldandroids.animation is requested, we cancel any previous postings of the Runnable
    ...
    /**
     * This class holds information about the overall com.nineoldandroids.animation being run on the set of
     ...
     * animator handling the com.nineoldandroids.animation. On every update event for an Animator, we ask the
    ...
    /**
     * This is the information we need to set each property during the com.nineoldandroids.animation.
     * mNameConstant is used to set the appropriate field in View, and the from/delta
     * values are used to calculate the animated value for a given com.nineoldandroids.animation fraction
     * during the com.nineoldandroids.animation.
     */
     ...
     * calculate and set the property during the com.nineoldandroids.animation. This structure is added to the
     * pending animations, awaiting the eventual start() of the underlying animator. A
     * Runnable is posted to start the com.nineoldandroids.animation, and any pending such Runnable is canceled
     ...
     * details of adding a pending com.nineoldandroids.animation and posting the request to start the com.nineoldandroids.animation.
     *
     * @param constantName The specifier for the property being animated
        ...
        if (mAnimatorMap.size() > 0) {
            ...
            for (Animator runningAnim : animatorSet) {
                ...
                if (bundle.cancel(constantName)) {
                    // property was canceled - cancel the com.nineoldandroids.animation if it's now empty
                    // Note that it's safe to break out here because every new com.nineoldandroids.animation
                    // on a property will cancel a previous com.nineoldandroids.animation on that property, so
                    // there can only ever be one such com.nineoldandroids.animation running.
                    if (bundle.mPropertyMask == NONE) {
                        // the com.nineoldandroids.animation is no longer changing anything - cancel it
     ...
     * property and then set it on the com.nineoldandroids.view object).
     */
    private class AnimatorEventListener
        ...
        public void onAnimationEnd(Animator animation) {
            ...
            // If the map is empty, it means all com.nineoldandroids.animation are done or canceled, so the listener
        ...
        /**
         * Calculate the current value for each property and set it on the com.nineoldandroids.view. Invalidate
         * the com.nineoldandroids.view object appropriately, depending on which properties are being animated.
         *
         * @param animation The animator associated with the properties that need to be
         * set. This animator holds the com.nineoldandroids.animation fraction which we will use to calculate
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            ...
            // logic is dependent on how the com.nineoldandroids.view handles an internal call to onSetAlpha().
